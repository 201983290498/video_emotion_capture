...
class SiglipVisionTower(nn.Module):
    ...
    def __init__(self, vision_tower, args, delay_load=False):
        ...
        self.select_feature = getattr(args, 'mm_vision_select_feature', 'patch')   # 特征类型（仅支持patch）
        if not delay_load:
            self.load_model()    # 立即加载
        else:
            self.cfg_only = SiglipVisionConfig.from_pretrained(self.vision_tower_name)    # 延迟加载仅存配置

    def load_model(self):
        self.image_processor = SiglipImageProcessor.from_pretrained(self.vision_tower_name)

        self.vision_tower = SiglipVisionModel.from_pretrained(self.vision_tower_name)
        self.vision_tower.requires_grad_(False)     # 冻结参数

        self.is_loaded = True

    def feature_select(self, image_forward_outs):
        image_features = image_forward_outs.hidden_states[self.select_layer]     # 选择指定层隐藏状态
        if self.select_feature == 'patch':
            image_features = image_features
        else:
            raise ValueError(f'Unexpected select feature: {self.select_feature}')
    ...
    def forward(self, images, raw_datas=None):
        if type(images) is list:     # 处理图像列表
            image_features = []
            for image in images:
                # 单张图像添加batch维度，输入模型并获取隐藏状态
                image_forward_out = self.vision_tower(image.to(device=self.device, dtype=self.dtype).unsqueeze(0), output_hidden_states=True)
                image_feature = self.feature_select(image_forward_out).to(image.dtype)  # 筛选目标层特征
                image_features.append(image_feature)
        else:   # 处理单张图像（batch输入）
            image_forward_outs = self.vision_tower(images.to(device=self.device, dtype=self.dtype), output_hidden_states=True)
            image_features = self.feature_select(image_forward_outs).to(images.dtype)
    ...
    def forward(self, samples):
        if isinstance(samples, list):
            ...
            for sample in samples:
                # 调用编码器，获取隐藏状态
                audio_forward_outs = self.audio_tower.encoder(sample.unsqueeze(0), output_hidden_states=True)
                # audio_feature = self.feature_select(audio_forward_outs).to(audio_features.dtype)
                audio_features.append(audio_forward_outs.last_hidden_state)  # 直接使用最后一层隐藏状态
        else:
...
# 根据视觉塔名称自动选择 CLIP 或 SigLIP 实现，返回实例
