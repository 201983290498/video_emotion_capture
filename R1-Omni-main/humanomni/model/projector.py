...
def build_audio_projector(config, delay_load=False, **kwargs):
    ...
    # 匹配形如"mlpNx_gelu"的投影器类型（N为正整数，代表MLP层数）
    mlp_gelu_match = re.match(r'^mlp(\d+)x_gelu$', projector_type)
    if mlp_gelu_match:
        mlp_depth = int(mlp_gelu_match.group(1))  # 提取MLP深度
        modules = [nn.Linear(config.audio_hidden_size, config.hidden_size)]  # 第一层：音频维度→目标维度
        # 后续层：保持目标维度，加入GELU非线性激活
        for _ in range(1, mlp_depth):
            modules.append(nn.GELU())
            modules.append(nn.Linear(config.hidden_size, config.hidden_size))
        return nn.Sequential(*modules)
...
class AllInOneSmall(nn.Module):
    ...
    def __init__(self, config, downsample=(2, 2, 2), depth=2, mlp_depth=2):
        ...
        """
        super().__init__()
        self.encoder_hidden_size = encoder_hidden_size = config.mm_hidden_size  # 输入特征维度
        self.hidden_size = hidden_size = config.hidden_size   # 中间层特征维度
        self.output_hidden_size = output_hidden_size = config.hidden_size   # 输出特征维度
        # TODO: make these as config arguments
        self.depth = depth    # 空间交互块深度
        self.mlp_depth = mlp_depth    # MLP投影器深度
        # 1. 第一阶段卷积块（视频特征）
        if depth != 0:
            self.s1_video = RegStage(
                depth=depth,
                ...
                out_chs=hidden_size,
                stride=1,     # 步长（不改变空间尺寸）
                dilation=1,   #  dilation（不扩张卷积）
                act_layer=nn.SiLU,
                norm_layer=LayerNorm2d,  # 归一化层（2D空间归一化）
            )
            # 1. 第一阶段卷积块（身体特征，与视频共享结构）
            self.s1_body = RegStage(
                depth=depth,
                in_chs=encoder_hidden_size,
                out_chs=hidden_size,
    ...
    def forward(self, video, body, face):
        ...
        """
       # import ipdb;ipdb.set_trace()
        t = video.size(1)   # 时间维度（帧数）

        # -------------------------- 处理视频特征 --------------------------
        # 1. 调整视频特征形状为3D卷积所需的格式 [b, d, t, h, w]
        if video.ndim == 4:
            hw = int(video.size(2) ** 0.5)
            x = einops.rearrange(video, "b t (h w) d -> b d t h w", h=hw, w=hw)    # 重排为[b, d, t, h, w]
        elif video.ndim == 5:
            x = einops.rearrange(video, "b t h w d -> b d t h w")
        # 2. 第一阶段卷积提取（合并batch和时间维度，便于2D卷积处理）
        x = einops.rearrange(x, "b d t h w -> (b t) d h w")
        x = self.s1_video(x)
        # 3. 恢复时间维度并进行下采样
        x = einops.rearrange(x, "(b t) d h w -> b d t h w", t=t)
        x = self.sampler_video(x)  # 3D卷积下采样：空间维度减半，时间维度通过padding保持
        x = x[:, :, :-1, :, :]
      #  print(x.shape)
        new_t = x.size(2)
       # 4. 第二阶段卷积提取与读出
        x = einops.rearrange(x, "b d t h w -> (b t) d h w")
        x = self.s2_video(x)   # 第二阶段卷积处理
        x = einops.rearrange(x, "(b t) d h w -> b (t h w) d", t=new_t)
        video_feature = self.readout_video(x)   # MLP投影到输出维度
        # -------------------------- 处理身体特征（与视频流程完全一致） --------------------------
        if body.ndim == 4:
            hw = int(body.size(2) ** 0.5)
            x = einops.rearrange(body, "b t (h w) d -> b d t h w", h=hw, w=hw)
      ...
      #  print(x.shape)
        ...
        body_feature = self.readout_body(x)
        # -------------------------- 处理面部特征（直接通过MLP投影） --------------------------
        mlp_res = self.mlp_2xgelu_face(face)  # 面部特征无需时空处理，直接映射
        return video_feature, body_feature, mlp_res
