...
class AudioModel(PreTrainedModel):
    ...
    def forward(self, audio, attention_mask=None, output_all_encoded_layers=True):
        ...
        # 确保attention_mask与audio具有相同的序列长度
        if attention_mask.size(1) != audio.size(1):
            # 如果长度不匹配，截断或填充attention_mask
            if attention_mask.size(1) > audio.size(1):
                attention_mask = attention_mask[:, :audio.size(1)]
            else:
                # 如果需要填充，添加0
                pad_size = audio.size(1) - attention_mask.size(1)
