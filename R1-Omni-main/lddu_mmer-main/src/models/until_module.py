...
class CTCModule(nn.Module):
    def __init__(self, in_dim, out_seq_len):
        ...
        # 使用LSTM进行特征提取，保持输入输出维度一致
        self.lstm = nn.LSTM(in_dim, in_dim, num_layers=2, batch_first=True)
        
        # 投影层将特征映射到目标序列长度+1（包括blank）
        self.projection = nn.Linear(in_dim, out_seq_len + 1)
    ...
    def forward(self, x):
        ...
        # 投影到目标维度
        pred_output_position_inclu_blank = self.projection(lstm_out)
        ...
        # 转置以进行对齐计算 [batch_size, out_seq_len, in_seq_len]
